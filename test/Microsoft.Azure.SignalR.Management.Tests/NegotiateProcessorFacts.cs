...
public class NegotiateProcessorFacts
{
    ...
    private static readonly Claim[] _defaultClaims = [new Claim("type1", "val1")];
    ...
    public async Task GenerateClientEndpoint(string userId, Claim[] claims, string appName)
    {
        ...
        for (var i = 0; i < 3; i++)
        {
            ...
            var expectedToken = JwtTokenHelper.GenerateJwtToken(ClientEndpointUtils.GetExpectedClientEndpoint(HubName, appName, endpoints[i].Endpoint),
                                                                ClaimsUtility.BuildJwtClaims(null, userId, () => claims),
                                                                token.ValidTo,
                                                                token.ValidFrom,
                                                                token.ValidFrom,
                                                                endpoints[i].AccessKey);
