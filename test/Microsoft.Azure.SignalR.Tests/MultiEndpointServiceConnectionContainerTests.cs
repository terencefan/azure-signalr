...
public class MultiEndpointServiceConnectionContainerTests : VerifiableLoggedTest
{
    ...
    private readonly string _connectionString3 = string.Format(ConnectionStringFormatter, Url3);

    public static IEnumerable<object[]> TestReloadEndpointsData { get; } = new object[][]
                                        {
        // no change
        new object[]
        {
    ...
    public async Task TestStatusPingChangesEndpointStatus()
    {
        using (StartVerifiableLog(out var loggerFactory, LogLevel.Trace))
        {
            var endpoints = new[]
            {
                new ServiceEndpoint(_connectionString1, EndpointType.Primary, "strong"),
                new ServiceEndpoint(_connectionString2, EndpointType.Secondary, "weak"),
            };
    ...
    public async Task TestContainerWithTwoEndpointWithOneOfflineSucceeds()
    {
        using (StartVerifiableLog(out var loggerFactory, LogLevel.Warning))
        {
            ...
            var container = new TestMultiEndpointServiceConnectionContainer("hub", e => string.IsNullOrEmpty(e.Name)
                ? (containers[e] = new TestServiceConnectionContainer(new List<IServiceConnection>
                {
                ...
                : (IServiceConnectionContainer)(containers[e] = new TestServiceConnectionContainer(new List<IServiceConnection>
                {
                    new TestSimpleServiceConnection(writeAsyncTcs: writeTcs),
                    new TestSimpleServiceConnection(writeAsyncTcs: writeTcs),
                    new TestSimpleServiceConnection(writeAsyncTcs: writeTcs),
    ...
    public async Task TestMultiEndpointConnectionWithNotExistEndpointRouter()
    {
        using (StartVerifiableLog(out var loggerFactory, LogLevel.Warning))
        {
            ...
            var container = new TestMultiEndpointServiceConnectionContainer("hub", e => string.IsNullOrEmpty(e.Name)
                ? new TestServiceConnectionContainer(new List<IServiceConnection>
                {
                    new TestSimpleServiceConnection(ServiceConnectionStatus.Disconnected),
                    new TestSimpleServiceConnection(ServiceConnectionStatus.Disconnected),
                    new TestSimpleServiceConnection(ServiceConnectionStatus.Disconnected),
                ...
                : (IServiceConnectionContainer)new TestServiceConnectionContainer(new List<IServiceConnection>
                {
                    new TestSimpleServiceConnection(),
                    new TestSimpleServiceConnection(),
                    new TestSimpleServiceConnection(),
    ...
    public async Task TestMultipleEndpointWithRenamesAndWriteAckableMessage()
    {
        var sem = new TestServiceEndpointManager(
            new ServiceEndpoint(_connectionString1, EndpointType.Primary, "1"),
            new ServiceEndpoint(_connectionString2, EndpointType.Primary, "2"),
            new ServiceEndpoint(_connectionString3, EndpointType.Secondary, "3")
            );
        ...
        {
            new ServiceEndpoint(_connectionString1, EndpointType.Primary, "11"),
            new ServiceEndpoint(_connectionString2, EndpointType.Primary, "2"),
            new ServiceEndpoint(_connectionString3, EndpointType.Secondary, "33")
        };
    ...
    public async Task TestMultipleEndpointWithClientEndpointUpdates()
    {
        ...
        var connstr3 = $"{_connectionString3};ClientEndpoint={testClientEp};";
    ...
    public async Task TestEndpointManagerWithAddEndpointsWithTimeoutCanPromote()
    {
        ...
        var newEndpoints = new ServiceEndpoint[]
        {
            new ServiceEndpoint(_connectionString1, EndpointType.Primary, "1"),
            new ServiceEndpoint(_connectionString2, EndpointType.Primary, "2"),
            new ServiceEndpoint(_connectionString3, EndpointType.Primary, "3")
        };
    ...
    public async Task TestEndpointManagerWithRemoveEndpointsWithNoClients()
    {
        var sem = new TestServiceEndpointManager(
            new ServiceEndpoint(_connectionString1, EndpointType.Primary, "1"),
            new ServiceEndpoint(_connectionString2, EndpointType.Primary, "22")
            );
        var endpoints = sem.Endpoints.Keys.ToArray();
        Assert.Equal(2, endpoints.Length);
        Assert.Equal("1", endpoints[0].Name);
    ...
    public async Task TestEndpointManagerWithMultiHubsWithEndpointTypeUpdate()
    {
        var sem = new TestServiceEndpointManager(
            new ServiceEndpoint(_connectionString1, EndpointType.Primary, "1"),
            new ServiceEndpoint(_connectionString2, EndpointType.Secondary, "22")
            );
        var endpoints = sem.Endpoints.Keys.ToArray();
        Assert.Equal(2, endpoints.Length);
        Assert.Equal("1", endpoints[0].Name);
    ...
    public async Task TestEndpointManagerWithMultiHubsWithServerEndpointUpdate()
    {
        ...
        var con1Se = $"{_connectionString1};ServerEndpoint={testSe};";
        var newEndpoints = new ServiceEndpoint[]
        {
        ...
        using (StartVerifiableLog(out var loggerFactory))
        {
            ...
            try
            {
                ...
                var taskCt1 = Task.Run(async () =>
                {
                    ...
                    await transportConnection1.Application.Output.WriteAsync(
                        protocol.GetMessageBytes(
                            new ConnectionDataMessage("client1", new ReadOnlySequence<byte>(" "u8.ToArray()))));
                    await Task.Delay(100);
                ...
                var taskCt2ReceivePings = Task.Run(async () =>
                {
                    ...
                    await transportConnection2.Application.Output.WriteAsync(
                        protocol.GetMessageBytes(
                            new ConnectionDataMessage("client2", new ReadOnlySequence<byte>("  "u8.ToArray()))));
                    await transportConnection22.Application.Output.WriteAsync(
                        protocol.GetMessageBytes(
                            new ConnectionDataMessage("client22", new ReadOnlySequence<byte>("   "u8.ToArray()))));
                    await Task.Delay(100);
    ...
    private static TestMultiEndpointServiceConnectionContainer CreateMultiEndpointConnection(EndpointStatus[] status, TaskCompletionSource<object> writeTcs, ILoggerFactory loggerFactory)
    {
        ...
        var containers = new Dictionary<ServiceEndpoint, TestServiceConnectionContainer>();
        return new TestMultiEndpointServiceConnectionContainer("hub", e => e.Name == EndpointStatus.Offline.ToString()
            ? (containers[e] = new TestServiceConnectionContainer(new List<IServiceConnection> {
                new TestSimpleServiceConnection(ServiceConnectionStatus.Disconnected),
                new TestSimpleServiceConnection(ServiceConnectionStatus.Disconnected),
                new TestSimpleServiceConnection(ServiceConnectionStatus.Disconnected),
            ...
            new TestSimpleServiceConnection(writeAsyncTcs: writeTcs),
            new TestSimpleServiceConnection(writeAsyncTcs: writeTcs),
            }, e, new AckHandler(100, 1000))), sem, router, loggerFactory);
    }
    ...
    private class TestEndpointRouter : EndpointRouterDecorator
    {
        ...
        {
            return _broken ? throw _ex : base.GetEndpointsForBroadcast(endpoints);
        }

        public override IEnumerable<ServiceEndpoint> GetEndpointsForConnection(string connectionId, IEnumerable<ServiceEndpoint> endpoints)
        {
            return _broken ? throw _ex : base.GetEndpointsForConnection(connectionId, endpoints);
        }

        public override IEnumerable<ServiceEndpoint> GetEndpointsForGroup(string groupName, IEnumerable<ServiceEndpoint> endpoints)
        {
            return _broken ? throw _ex : base.GetEndpointsForGroup(groupName, endpoints);
        }

        public override IEnumerable<ServiceEndpoint> GetEndpointsForUser(string userId, IEnumerable<ServiceEndpoint> endpoints)
        {
            return _broken ? throw _ex : base.GetEndpointsForUser(userId, endpoints);
        }

        public override ServiceEndpoint GetNegotiateEndpoint(HttpContext context, IEnumerable<ServiceEndpoint> endpoints)
        {
            return _broken ? throw _ex : base.GetNegotiateEndpoint(context, endpoints);
        }
    }
}