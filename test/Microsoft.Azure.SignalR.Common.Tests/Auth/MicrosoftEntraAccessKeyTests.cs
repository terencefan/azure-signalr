...
public class MicrosoftEntraAccessKeyTests
{
    ...
    public async Task ThrowUnauthorizedExceptionTest()
    {
        ...
        Assert.StartsWith("API failed with status code 401.", ex.Message);
        Assert.EndsWith($"Request Uri: {endpoint}", ex.Message);
    }

    [Theory]
    [InlineData(AzureSignalRForbiddenException.IngressDenied, "nginx")]
    [InlineData(AzureSignalRForbiddenException.RuntimeDenied, "http")]
    public async Task ThrowForbiddenExceptionTest(string expected, string responseContent)
    {
        ...
        Assert.False(key.IsAuthorized);
        var ex = Assert.IsType<AzureSignalRForbiddenException>(key.LastException);
        Assert.StartsWith("API failed with status code 403.", ex.Message);
